...
class _ImageViewerScreenState extends State<ImageViewerScreen> {
  ...
  Future<void> _shareImage() async {
    try {
      ...
      ], text: widget.title ?? AppLocalizations.of(context).shareImage);
  ...
  void _toggleZoom() {
    if (_isZoomed) {
      ...
      _transformationController.value = Matrix4.identity().scaled(2.0);
...
//     try {
//       // Get image file from cache or download it
//       final File file = await DefaultCacheManager().getSingleFile(
//         widget.imageUrl,
//       );
...
//       // Create a temporary file with a proper extension for sharing if needed,
//       // but often the cached file path is sufficient.
//       // However, to be safe and consistent with previous behavior (renaming):
//       final tempDir = await getTemporaryDirectory();
//       final shareFile = File('${tempDir.path}/share_image.jpg');
//       await shareFile.writeAsBytes(await file.readAsBytes());
