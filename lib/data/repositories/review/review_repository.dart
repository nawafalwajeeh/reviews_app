...
class ReviewRepository extends GetxController {
  ...
  Future<void> updateReview(ReviewModel review) async {
    try {
      ...
      final oldReviewDoc = await _db.collection(_collectionName).doc(review.id).get();
      final oldRating = (oldReviewDoc.data()?['rating'] as num?)?.toInt();
      final newRating = review.rating.round();

      // Update the review
      await _db.collection(_collectionName).doc(review.id).update(review.toJson());
  ...
  Future<void> deleteReview(String reviewId) async {
    try {
      ...
      final reviewDoc = await _db.collection(_collectionName).doc(reviewId).get();
  ...
  Future<void> updateRatingDistribution(String placeId, int newRating) async {
    try {
      ...
      await _db.runTransaction((transaction) async {
        ...
        final currentDistribution = placeDoc.data()?['RatingDistribution'] ?? {
          '5': 0, '4': 0, '3': 0, '2': 0, '1': 0
        };
        ...
        updatedDistribution[ratingKey] = (updatedDistribution[ratingKey] ?? 0) + 1;
  ...
  Future<void> removeFromRatingDistribution(String placeId, int oldRating) async {
    try {
      final placeRef = _db.collection('Places').doc(placeId);
      ...
      await _db.runTransaction((transaction) async {
        ...
        updatedDistribution[ratingKey] = currentCount > 0 ? currentCount - 1 : 0;
  ...
  Future<Map<String, int>> getRatingDistribution(String placeId) async {
    try {
      ...
      final distribution = placeDoc.data()?['RatingDistribution'] ?? {
        '5': 0, '4': 0, '3': 0, '2': 0, '1': 0
      };
...
//         final updatedDistribution = Map<String, int>.from(currentDistribution);
//         updatedDistribution[newRating.toString()] =
//             (updatedDistribution[newRating.toString()] ?? 0) + 1;
...
//       });
//     } catch (e) {
//       throw Exception('Failed to update rating distribution: $e');
//     }
//   }
// }
