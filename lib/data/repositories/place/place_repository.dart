...
class PlaceRepository extends GetxController {
  ...
  Future<void> _addImagesToGallery(PlaceModel place) async {
    final batch = _db.batch();
    final galleryCollection = _db.collection('GalleryImages');

    for (var imageUrl in place.images!) {
      // Create a new GalleryImageModel for each image
      final galleryImage = GalleryImageModel(
        id: '', // ID will be generated by Firestore
        imageUrl: imageUrl,
        // Using the place's CategoryId as the CollectionId for future filtering/grouping
        collectionId: place.categoryId,
        timestamp: DateTime.now(),
        placeId: place.id, // Link back to the place
        placeName: place.title, // Save place name for easy lookups
      );

      // Add operation to the batch
      batch.set(galleryCollection.doc(), galleryImage.toJson());
    }
  ...
  // Method used by GalleryController for Featured Images
  Future<List<PlaceModel>> getRecentPlaces({int limit = 5}) async {
    try {
      final snapshot = await _db
          .collection('Places')
          .orderBy(
            'Timestamp',
            descending: true,
          ) // Assuming PlaceModel has a Timestamp
          .limit(limit)
          .get();
